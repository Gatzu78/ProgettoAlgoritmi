...
int loadFile(FILE *fileptr, long *lungfile, char *filePath) {
    /*
    Controllo se il file è vuoto.
    */
    if (!(fileptr = fopen(filePath, "rb"))){ // apertura il file in modalità binaria
        ...
        *lungfile = ftell(fileptr) + 1;        // memorizza l'offset del file (quanti caratteri ci sono) + 1
        ...
        fileBuffer = (unsigned char *) malloc((*lungfile)*sizeof(unsigned char));    // alloca abbastanza memoria per contenere il file previsto  anche \0
        fread(fileBuffer, (size_t)*lungfile,1,fileptr);                              // legge il file e lo carica in memoria (copia sul buffer)
